...
__kernel void mat_mul(__global float *A, __global float *B, __global float *C, int N, int M, int K)
{
  ...
  for(int k=0;k<num;k++)
  {
    ...
    tmpB[x][y] = B[K*(BLOCK_SIZE*k+x) + BLOCK_SIZE*j+y];
  ...
  C[K*(BLOCK_SIZE*i+x) + BLOCK_SIZE*j+y] = acc.x+acc.y+acc.z+acc.w;
}
